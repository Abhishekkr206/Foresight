import { Pulse, Waveform, CaretLeft, CaretRight, MapPin, PaperPlaneTilt, X } from '@phosphor-icons/react';
    ...
    {open && <>
      ...
      <div className="assistant-messages mt-6 flex-1 min-h-0 space-y-3 overflow-y-auto pr-1">
        ...
        {messages.map((message, index) => <div key={index} className={"assistant-message rounded-xl p-3.5 " + (message.role === "user" ? "assistant-message-user ml-8" : "assistant-message-ai mr-2")}>
          ...
          {message.role === "assistant" && message.highlightedEventCount > 0 && <p className="mt-3 flex items-center gap-1.5 text-xs text-slate-600"><Pulse weight="duotone" size={13} className="text-moss-500" /> {message.highlightedEventCount} matching event{message.highlightedEventCount === 1 ? "" : "s"} highlighted</p>}
        </div>)}
      </div>
