import { Activity, BatteryCharging, ChevronLeft, ChevronRight, Radio } from 'lucide-react';
    ...
    <div className="sidebar-scroll flex-1 overflow-auto p-4">
      {open ? <>
        <div className="sidebar-health rounded-2xl p-4">
          ...
          <div className="mt-3 flex items-center gap-2 text-xs text-slate-600"><Activity size={14} /> Soundscape stable</div>
        </div>
        ...
        <div className="mt-6"><div className="flex items-end justify-between"><p className="text-[10px] font-semibold tracking-[.16em] text-slate-500">BEACONS</p><span className="text-xs text-slate-500">{nodes.length} nodes</span></div>
          <div className="mt-3 space-y-2">{nodes.map((beacon) => <div key={beacon.beacon_id}><button type="button" onClick={() => onBeaconSelect(beacon.beacon_id)} className={`beacon-sidebar-row flex w-full items-center gap-3 rounded-xl px-3 py-3 text-left ${beacon.threat_active ? 'beacon-sidebar-row-threat' : selectedBeaconId === beacon.beacon_id ? 'beacon-sidebar-row-selected' : highlightedBeaconIds.includes(beacon.beacon_id) ? 'beacon-sidebar-row-ai' : ''}`}><span className={`h-2 w-2 rounded-full ${beacon.threat_active ? 'bg-red-600' : beacon.severity === 'warning' ? 'bg-orange-400' : beacon.active === false ? 'bg-slate-300' : 'bg-moss-500'}`}></span><div className="min-w-0 flex-1"><p className="truncate text-xs font-semibold text-slate-900">{beacon.beacon_id}</p><p className="mt-1 text-[10px] text-slate-500">{beacon.is_real_hardware ? 'Hardware' : 'Simulated'}</p></div><span className="flex items-center gap-1 text-[10px] text-slate-500">{beacon.battery_percentage !== undefined && <BatteryCharging size={12} />}{beacon.battery_percentage !== undefined ? `${Math.round(beacon.battery_percentage)}%` : 'Ready'}</span></button>{selectedBeaconId === beacon.beacon_id && <div className="pl-2"><BeaconAudioPlayer beaconId={beacon.beacon_id} /></div>}</div>)}</div>
        </div>
      </> : <div className="sidebar-collapsed-status space-y-5 pt-5 text-center"><Radio size={16} className="mx-auto text-moss-500" /><span className="block text-[10px] font-semibold text-slate-500">{Math.round(health)}%</span>{nodes.map((beacon) => <span key={beacon.beacon_id} className={`mx-auto block h-2 w-2 rounded-full ${beacon.active === false ? 'bg-slate-300' : 'bg-moss-500'}`} title={beacon.beacon_id}></span>)}</div>}
    </div>
  </aside>;
}
