...
export default function BeaconMap({ beacons, simulator, selectedZone, setSelectedZone, selectedEvent, relatedBeacons, rippleBeacon, highlightedBeacons, selectedBeaconId, onBeaconSelect }) {
  ...
  return <div className="relative"><div className="selected-chunk-label absolute bottom-3 left-3 z-[1000] flex items-center gap-2 rounded-lg border border-slate-200 bg-white px-3 py-2 text-xs shadow-sm"><span className="font-semibold text-slate-500">SELECTED CHUNK</span><span className="font-bold text-moss-500">{selectedZone || 'No chunk selected'}</span></div><MapContainer className="h-[560px] w-full rounded-xl" bounds={[[BOUNDS.min_lat, BOUNDS.min_lon], [BOUNDS.max_lat, BOUNDS.max_lon]]} maxBounds={[[BOUNDS.min_lat - 0.02, BOUNDS.min_lon - 0.02], [BOUNDS.max_lat + 0.02, BOUNDS.max_lon + 0.02]]} scrollWheelZoom>
    ...
    {zones.map((zone) => {
      const threat = simulator?.zones?.[zone.id]?.threat;
      return <Rectangle key={zone.id} bounds={zone.bounds} pathOptions={{ color: selectedZone === zone.id ? '#588157' : '#A3B18A', weight: selectedZone === zone.id ? 3 : 1, fillOpacity: selectedZone === zone.id ? 0.25 : threat ? 0.2 : 0.07 }} eventHandlers={{ click: () => setSelectedZone(zone.id) }} />;
    })}
    {beacons.map((beacon) => {
      ...
      const score = selectedEvent?.event?.beacon_id === beacon.beacon_id ? selectedEvent.event.final_score : 0;
      const color = score >= THREAT_THRESHOLD ? '#A3B18A' : '#588157';
      return <React.Fragment key={beacon.beacon_id}>
        {(rippleBeacon === beacon.beacon_id || selected) && <CircleMarker center={[beacon.latitude, beacon.longitude]} radius={directSelected ? 22 : 15} pathOptions={{ color, fillColor: color, fillOpacity: 0, opacity: directSelected ? 0.75 : 0.42, weight: directSelected ? 2 : 1, className: rippleBeacon === beacon.beacon_id ? 'beacon-ripple' : '' }} />}
        <CircleMarker center={[beacon.latitude, beacon.longitude]} radius={beacon.is_real_hardware ? 10 : 8} pathOptions={{ color, fillColor: color, fillOpacity: 0.78, weight: directSelected ? 4 : selected ? 3 : 2, className: score >= THREAT_THRESHOLD ? 'beacon-pulse' : '' }} eventHandlers={{ click: () => onBeaconSelect(beacon.beacon_id) }}>
          <Popup><strong>{beacon.beacon_id}</strong><br />{beacon.is_real_hardware ? 'Real hardware' : `Simulated / ${zoneFor(beacon)}`}{directSelected && <><br />Selected beacon</>}{!directSelected && aiHighlighted && <><br />AI matched</>}</Popup>
        </CircleMarker>
      </React.Fragment>;
    })}
  </MapContainer></div>;
