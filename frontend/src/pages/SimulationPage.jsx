...
export default function SimulationPage() {
  ...
  const selectedBeaconIds = selectedState?.beacon_ids || [];
    ...
    <main className="dashboard-content simulation-content mx-auto max-w-5xl px-4 py-6 sm:px-8 sm:py-10">
      ...
      <section className="rounded-2xl border border-slate-200 bg-white p-5 shadow-xl sm:p-7">
        ...
        <div className="grid gap-5 md:grid-cols-2">
          ...
          <div className="rounded-lg border border-slate-200 bg-slate-50 px-3 py-3 text-xs text-slate-600"><span className="font-semibold text-slate-800">Selected zone state</span><span className="mt-1 block">{selectedState?.threat ? selectedState.sound + ' active · ' + remaining + 's left' : 'Ambient loop'}</span>{selectedBeaconIds.length > 0 && <span className="mt-1 block">Beacons: {selectedBeaconIds.join(', ')}</span>}{selectedBeaconIds.length > 1 && <span className="mt-1 block text-amber-700">Multiple beacons receive this scenario.</span>}</div>
        </div>
